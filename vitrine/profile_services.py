...
def load_profile_state(
    workspace_root: str | Path,
) -> tuple[PortfolioProfileState, int | None]:
    ...
    if issues:
        raise ProfileWorkflowError(
            ...
            f"Portfolio Profile state contains {len(issues)} issue(s).",
        )
    ...
    *,
    ...
    try:
        return commit_record_batch(
            workspace_root, records, expected_state_revision=expected_state_revision
        )
    except VitrineStorageConflictError as error:
        raise ProfileWorkflowError("state_conflict", str(error)) from error


def list_profile_families(
    workspace_root: str | Path,
) -> tuple[PortfolioProfileFamily, ...]:
...
def list_profile_revisions(
    workspace_root: str | Path, *, portfolio_profile_id: str | None = None
) -> tuple[ProfileRevisionSummary, ...]:
    state, _ = load_profile_state(workspace_root)
    revisions = (
        state.revisions
        if portfolio_profile_id is None
        else tuple(
            item
            for item in state.revisions
            if item.portfolio_profile_id == portfolio_profile_id
        )
    )
    return tuple(
        ...
        for item in sorted(
            revisions,
            key=lambda item: (item.portfolio_profile_id, item.profile_revision),
        )
    )


def get_profile_revision(
    workspace_root: str | Path, reference: ProfileRevisionRef
) -> PortfolioProfileRevision:
    state, _ = load_profile_state(workspace_root)
    ...
    if revision is None:
        raise ProfileWorkflowError(
            ...
            "Exact Portfolio Profile Revision does not exist.",
        )
    return revision
...
def list_bindable_profile_revisions(
    workspace_root: str | Path, *, purpose_kind: str | None = None
) -> tuple[ProfileRevisionSummary, ...]:
    return tuple(
        item
        for item in list_profile_revisions(workspace_root)
        if item.bindable and (purpose_kind is None or item.purpose_kind == purpose_kind)
    )


def get_portfolio_profile_binding(
    workspace_root: str | Path, portfolio_id: str
) -> PortfolioProfileBinding | None:
    state, _ = load_profile_state(workspace_root)
...
def get_profile_migration_history(
    workspace_root: str | Path, portfolio_id: str
) -> tuple[PortfolioProfileMigration, ...]:
    state, _ = load_profile_state(workspace_root)
    return tuple(
        sorted(
            (item for item in state.migrations if item.portfolio_id == portfolio_id),
            key=lambda item: (item.migrated_at, item.profile_migration_id),
        )
    )
    ...
    *,
    ...
    existing = next(
        (
            ...
            if item.profile_family_id == family.profile_family_id
        ),
        None,
    )
    if existing is not None:
        ...
        raise ProfileWorkflowError(
            "profile_family_conflict",
            "Profile Family identity already exists with different content.",
        )
    commit = _commit(
        workspace_root, (family,), expected_state_revision=expected_state_revision
    )
    ...
    *,
    ...
    if revision.purpose_kind not in {"improvement", "showcase"}:
        raise ProfileWorkflowError(
            ...
            "#31 operational services support improvement and showcase Profile purposes only.",
        )
    if revision.profile_family_id is not None and not any(
        item.profile_family_id == revision.profile_family_id for item in state.families
    ):
        raise ProfileWorkflowError(
            "profile_family_not_found", "Profile Family does not exist."
        )
    if (
        revision.predecessor_revision is not None
        and state.revision(
            ProfileRevisionRef(
                portfolio_profile_id=revision.portfolio_profile_id,
                profile_revision=revision.predecessor_revision,
            )
        )
        is None
    ):
        raise ProfileWorkflowError(
            "profile_predecessor_invalid",
            "Exact predecessor Profile Revision does not exist.",
        )
    if not requirements:
        raise ProfileWorkflowError(
            "profile_incomplete",
            "Operational Profile Revision requires at least one explicit requirement.",
        )
    normalized = tuple(requirements)
    for requirement in normalized:
        if requirement.profile_reference != revision.reference:
            raise ProfileWorkflowError(
                "profile_requirement_revision_mismatch",
                "Every requirement must identify the exact Profile Revision being created.",
            )
    if len({item.requirement_id for item in normalized}) != len(normalized):
        raise ProfileWorkflowError(
            "duplicate_requirement_id",
            "Requirement IDs must be unique within the Profile Revision.",
        )
    candidate_state = project_profile_state(
        (*_records_and_revision(workspace_root)[0], revision, *normalized)
    )
    ...
    return ProfileMutationResult(
        (
            f"{revision.portfolio_profile_id}:{revision.profile_revision}",
            *(item.requirement_id for item in normalized),
        ),
        commit,
    )
...
def _lifecycle_head(
    state: PortfolioProfileState, reference: ProfileRevisionRef
) -> PortfolioProfileLifecycleEvent | None:
    heads = state.lifecycle_heads(reference)
    if len(heads) > 1:
        raise ProfileWorkflowError(
            "profile_lifecycle_conflict",
            "Profile Revision has multiple unresolved lifecycle heads.",
        )
    return heads[0] if heads else None
...
def activate_profile_revision(
    ...
    *,
    ...
    if revision.purpose_kind not in {"improvement", "showcase"}:
        raise ProfileWorkflowError(
            "profile_purpose_unsupported",
            "#31 may activate improvement and showcase Profile purposes only.",
        )
    if not state.requirements_for(reference):
        raise ProfileWorkflowError(
            "profile_incomplete",
            "Profile Revision has no explicit operational requirements.",
        )
    ...
    if head is not None:
        ...
        raise ProfileWorkflowError(
            ...
            f"Profile Revision lifecycle head is {head.event_kind}; activation is not implicit.",
        )
    now = clock()
    ...
    commit = _commit(
        workspace_root, (event,), expected_state_revision=expected_state_revision
    )
...
def transition_profile_lifecycle(
    ...
    *,
    ...
    if event_kind not in {"deprecated", "superseded", "withdrawn", "retired"}:
        raise ProfileWorkflowError(
            "profile_lifecycle_invalid", "Unsupported lifecycle transition."
        )
    state, actual = load_profile_state(workspace_root)
    _require_expected(actual, expected_state_revision)
    if state.revision(reference) is None:
    ...
    if event_kind == "superseded":
        if successor_revision is None or state.revision(successor_revision) is None:
            raise ProfileWorkflowError(
                "profile_revision_not_found",
                "Supersession requires an existing exact successor Revision.",
            )
    elif successor_revision is not None:
        raise ProfileWorkflowError(
            "profile_lifecycle_invalid", "Only supersession accepts successor_revision."
        )
    now = clock()
    event = PortfolioProfileLifecycleEvent(
        profile_lifecycle_event_id=id_factory("profile_event"),
...
def validate_profile_applicability(
    revision: PortfolioProfileRevision, context: ProfileBindingContext
) -> None:
    applicability = revision.applicability
    if (
        applicability.school_years
        and context.school_year not in applicability.school_years
    ):
        raise ProfileWorkflowError(
            "profile_not_applicable",
            "Binding context school year does not match Profile applicability.",
        )
    if (
        applicability.institution_id is not None
        and context.institution_id != applicability.institution_id
    ):
        raise ProfileWorkflowError(
            "profile_not_applicable",
            "Binding context institution does not match Profile applicability.",
        )
    if (
        applicability.program_id is not None
        and context.program_id != applicability.program_id
    ):
        raise ProfileWorkflowError(
            "profile_not_applicable",
            "Binding context program does not match Profile applicability.",
        )
    if (
        applicability.content_areas
        and context.content_area not in applicability.content_areas
    ):
        raise ProfileWorkflowError(
            "profile_not_applicable",
            "Binding context content area does not match Profile applicability.",
        )
    if applicability.effective_from is not None:
        if context.as_of is None or context.as_of < applicability.effective_from:
            raise ProfileWorkflowError(
                "profile_not_applicable",
                "Binding context is before Profile effective date or lacks an as-of date.",
            )
    if applicability.effective_through is not None:
        if context.as_of is None or context.as_of > applicability.effective_through:
            raise ProfileWorkflowError(
                "profile_not_applicable",
                "Binding context is after Profile effective date or lacks an as-of date.",
            )
...
def bind_portfolio_profile(
    ...
    *,
    ...
    if not state.is_bindable(reference):
        ...
        raise ProfileWorkflowError(
            f"profile_{status}",
            f"Profile Revision is not bindable: lifecycle status is {status}.",
        )
    validate_profile_applicability(revision, context)
    heads = state.active_binding_heads(portfolio_id)
    if heads:
        if len(heads) > 1:
            raise ProfileWorkflowError(
        ...
        raise ProfileWorkflowError(
            ...
            "Portfolio already has an active Profile Binding; migrate explicitly.",
        )
    ...
    commit = _commit(
        workspace_root, (binding,), expected_state_revision=expected_state_revision
    )
    return ProfileMutationResult((binding.profile_binding_id,), commit)


def compare_profile_requirements(
    source: Sequence[PortfolioProfileRequirement],
    target: Sequence[PortfolioProfileRequirement],
) -> ProfileRequirementImpact:
    ...
    for requirement_id in sorted(set(old) & set(new)):
        if requirement_semantic_key(old[requirement_id]) == requirement_semantic_key(
            new[requirement_id]
        ):
...
def analyze_profile_migration(
    ...
    *,
    ...
    if len(heads) != 1:
        ...
        raise ProfileWorkflowError(
            code,
            "Portfolio must have exactly one active Profile Binding before migration.",
        )
    current = heads[0]
    target_revision = state.revision(target)
    if target_revision is None:
        raise ProfileWorkflowError(
            "profile_revision_not_found", "Target Profile Revision does not exist."
        )
    if not state.is_bindable(target):
        raise ProfileWorkflowError(
            "profile_inactive", "Target Profile Revision is not explicitly bindable."
        )
    validate_profile_applicability(target_revision, context)
    ...
    changed_ids = set(
        (
            ...
            *impact.removed,
            *impact.replaced,
            *impact.materially_changed,
            *impact.unresolved_mapping,
        )
    )
        ...
        sorted(
            {
                ...
                for item in (*source_requirements, *target_requirements)
                if item.requirement_id in changed_ids
                and item.scope_kind == "section"
                and item.scope_reference is not None
            }
        )
    )
    reapproval = tuple(
        sorted(
            ...
            if item.requirement_kind == "approval"
            and item.requirement_id in changed_ids
        )
    )
    selection_count = sum(
        1
        for item in records
        if getattr(item, "record_type", None) == "portfolio_selection"
        and getattr(item, "portfolio_id", None) == portfolio_id
        and getattr(item, "profile_binding_id", None) == current.profile_binding_id
    )
...
def migrate_portfolio_profile(
    ...
    *,
    ...
    analysis = analyze_profile_migration(
        workspace_root, portfolio_id, target, context=context
    )
    if analysis.blocked:
        raise ProfileWorkflowError(
            "profile_migration_blocked",
            "Migration contains materially changed or unresolved requirement mappings.",
        )
    predecessor = next(
        item
    ...
    return analysis, ProfileMutationResult(
        (successor.profile_binding_id, migration.profile_migration_id), commit
    )


def create_profile_overlay(
    workspace_root: str | Path,
    ...
    *,
    ...
    for component in overlay.component_revisions:
        ...
        if component_revision is None:
            raise ProfileWorkflowError(
                "profile_revision_not_found",
                "Overlay component Profile Revision does not exist.",
            )
        if component_revision.purpose_kind != overlay.purpose_kind:
            raise ProfileWorkflowError(
                "profile_composition_conflict",
                "Overlay purpose must match every exact component Revision.",
            )
    if overlay.predecessor_overlay_revision is not None and not any(
        item.overlay_id == overlay.overlay_id
        and item.overlay_revision == overlay.predecessor_overlay_revision
        for item in state.overlays
    ):
        raise ProfileWorkflowError(
            "overlay_predecessor_invalid",
            "Overlay predecessor Revision does not exist.",
        )
    commit = _commit(
        workspace_root, (overlay,), expected_state_revision=expected_state_revision
    )
    return ProfileMutationResult(
        (f"{overlay.overlay_id}:{overlay.overlay_revision}",), commit
    )


def _overlay_to_requirement(
    item: ProfileOverlayRequirement, reference: ProfileRevisionRef
) -> PortfolioProfileRequirement:
...
def compose_profile_revision(
    ...
    *,
    ...
    if not component_revisions:
        raise ProfileWorkflowError(
            "profile_composition_conflict",
            "Composition requires at least one exact component Revision.",
        )
    ...
    for component_reference in component_revisions:
        ...
        if revision is None:
            raise ProfileWorkflowError(
                "profile_revision_not_found",
                "Composition component Profile Revision does not exist.",
            )
        if state.lifecycle_status(component_reference) not in {
            "activated",
            "deprecated",
        }:
            raise ProfileWorkflowError(
                "profile_inactive",
                "Composition component must be explicitly activated or deprecated.",
            )
        if revision.purpose_kind != effective_revision.purpose_kind:
            raise ProfileWorkflowError(
                "profile_composition_conflict",
                "Composition components must agree with effective purpose_kind.",
            )
        for requirement in state.requirements_for(component_reference):
            ...
            if existing_requirement is not None and requirement_semantic_key(
                existing_requirement
            ) != requirement_semantic_key(requirement):
                raise ProfileWorkflowError(
                    "profile_composition_conflict",
                    f"Component requirement conflict: {requirement.requirement_id}.",
                )
        ...
        for section in revision.sections:
            ...
            if existing_section is not None and existing_section != section:
                raise ProfileWorkflowError(
                    "profile_composition_conflict",
                    f"Component section conflict: {section.section_id}.",
                )
        ...
        for audience in revision.audience_rules:
            ...
            if existing_audience is not None and existing_audience != audience:
                raise ProfileWorkflowError(
                    "profile_composition_conflict",
                    f"Component audience conflict: {audience.audience_rule_id}.",
                )
    ...
    for overlay_reference in overlay_revisions:
        overlay = next(
            (
                item
                for item in state.overlays
                if item.overlay_id == overlay_reference.overlay_id
                and item.overlay_revision == overlay_reference.overlay_revision
            ),
            None,
        )
        if overlay is None:
            raise ProfileWorkflowError(
                "overlay_not_found", "Exact Overlay Revision does not exist."
            )
        if overlay.purpose_kind != effective_revision.purpose_kind:
            raise ProfileWorkflowError(
                "profile_composition_conflict",
                "Overlay purpose does not match the effective Profile purpose.",
            )
        if not set(overlay.component_revisions).issubset(set(component_revisions)):
            raise ProfileWorkflowError(
                "profile_composition_conflict",
                "Overlay was authored against component Revisions not present in this composition.",
            )
        for change in overlay.requirement_changes:
            ...
            if change.action == "add":
                if item.requirement_id in merged_requirements:
                    raise ProfileWorkflowError(
                        "profile_composition_conflict",
                        f"Overlay addition collides with requirement: {item.requirement_id}.",
                    )
                merged_requirements[item.requirement_id] = _overlay_to_requirement(
                    item, effective_revision.reference
                )
            else:
                ...
                if target_id is None or target_id not in merged_requirements:
                    raise ProfileWorkflowError(
                        "profile_composition_conflict",
                        f"Overlay replacement target is missing for {item.requirement_id}.",
                    )
                old = merged_requirements[target_id]
                if overlay.authority_reference not in old.authority_references:
                    raise ProfileWorkflowError(
                        "profile_composition_conflict",
                        f"Overlay authority is not explicitly permitted to replace requirement: {target_id}.",
                    )
                if _weakened(old, item):
                    raise ProfileWorkflowError(
                        "profile_composition_conflict",
                        f"Overlay replacement weakens controlling requirement: {target_id}.",
                    )
                if (
                    item.requirement_id != target_id
                    and item.requirement_id in merged_requirements
                ):
                    raise ProfileWorkflowError(
                        "profile_composition_conflict",
                        f"Overlay replacement collides with requirement: {item.requirement_id}.",
                    )
        ...
        for section in overlay.section_additions:
            if section.section_id in sections:
                raise ProfileWorkflowError(
                    "profile_composition_conflict",
                    f"Overlay section addition collides with section: {section.section_id}.",
                )
            sections[section.section_id] = section
        for audience in overlay.audience_rule_additions:
            if audience.audience_rule_id in audiences:
                raise ProfileWorkflowError(
                    "profile_composition_conflict",
                    f"Overlay audience addition collides with audience: {audience.audience_rule_id}.",
                )
            audiences[audience.audience_rule_id] = audience
    ...
    if (
        effective_revision.sections != expected_sections
        or effective_revision.audience_rules != expected_audiences
    ):
        raise ProfileWorkflowError(
            "profile_composition_conflict",
            "Effective Revision must contain the fully flattened component and overlay sections/audience rules.",
        )
    effective_requirements = tuple(
        replace(
            item,
            portfolio_profile_id=effective_revision.portfolio_profile_id,
            profile_revision=effective_revision.profile_revision,
        )
        for item in sorted(
            merged_requirements.values(), key=lambda item: item.requirement_id
        )
    )
    ...
    candidate_state = project_profile_state(
        (*_records_and_revision(workspace_root)[0], *candidate_records)
    )
    issues = collect_profile_state_issues(candidate_state)
    if issues:
        first = issues[0]
    ...
    return ProfileCompositionResult(
        effective_revision.reference,
        tuple(item.requirement_id for item in effective_requirements),
